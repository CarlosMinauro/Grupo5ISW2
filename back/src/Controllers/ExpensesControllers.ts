...
import { Expenses } from "../DAO/models";
...
const ExpensesController = (): [string, Router] => {
  ...
  router.get("/", async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const gastos = await Expenses.findAll({ where: { user_id: decodedToken.id } });
      res.json({ msg: "", gastos });
    } catch (error) {
  ...
  router.put("/:id", async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const gasto = await Expenses.findOne({ where: { id, user_id: decodedToken.id } });
      if (!gasto) {
  ...
  router.get("/", async (req: Request, res: Response):Promise<any> => {
    try {
      ...
      // 4) Buscar gastos
      const gastos = await Expenses.findAll({ where: whereClause });
