...
import { fetchExpenses, editGasto } from "../services/GastoService"
...
// Principio SRP: Este componente solo se encarga de la UI y orquestar la edición
// Principio DIP: Depende del servicio editGasto, no de la implementación concreta
const EditarGasto = ({ id, onClose, onUpdate}: Props) => {
  ...
  async function enviar() {
    if (!dato) return
    try {
      await editGasto(dato) // DIP: usa el servicio
      onUpdate()
      onClose()
    } catch (error) {
