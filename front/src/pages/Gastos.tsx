...
import { fetchExpenses, crearGasto } from "../services/GastoService";
...
import { fetchCategories, CategoriaTipo } from "../services/CategoryService";
...
function Gastos() {
  ...
  const addGastoHandler = async (
    ...
    // Principio SRP: Esta función solo prepara los datos y delega la creación al servicio
    // Principio DIP: Depende de la abstracción crearGasto, no de la implementación concreta de fetch
    const ng_date = new Date(ng_fecha);
    const gastoData = {
      date: ng_date.toISOString().split('T')[0], // Format as YYYY-MM-DD
      amount: ng_monto,
    ...
    try {
      ...
      await cargarGastos();
      verificarPresupuesto([...lista, gastoData as GastoTipo], ng_categoria);
